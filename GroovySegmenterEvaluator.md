...
Call with `pipeline <filename> [<language>]`, e.g. `pipeline testsuite.xml`.
...
The **language** parameter is optional. If given, only the tests for the specified language will be run.
...
// Avoid NoClassDefFoundError for "Messages"
@Grab(group='de.tudarmstadt.ukp.dkpro.core',
      module='de.tudarmstadt.ukp.dkpro.core.api.parameter-asl',
      version='1.7.0')
...
// See if optional language argument is given
def lang = args.length >=2 ? args[1] : null
println "Running tests for ${lang ? lang : 'all languages'}"
...
for (def tc : testSuite.tests.test) { // Run tests
  if (lang && tc.@language != lang) {
    println "Skipping test for unselected language ${tc.@language}"
    continue    
  }
